    ...
    $.ajax({
      ...
      success: function (respuesta) {
        ...
        $(nuevaCantidadProducto).attr(
          "nuevoStock",
          Number(respuesta["stock"]) - nuevaCantidadProducto
        );
...
$(".formularioVenta ").on("change", "input.nuevaCantidadProducto", function () {
  ...
  var idProducto1 = $(
    ".formularioVenta ." +
      nombreDiv.attr("class") +
      " input.nuevaDescripcionProducto"
  ).attr("idProducto");
  ...
  $(
    ".formularioVenta ." +
      nombreDiv.attr("class") +
      " input.nuevoPrecioProducto"
  ).val(
    $(this).val() *
      $(
        ".formularioVenta ." +
          nombreDiv.attr("class") +
          " input.nuevoPrecioUnitarioProducto"
      ).val()
  );
  // SUMAR TOTAL DE PRECIOS
...
function sumarTotalPrecios() {
  ...
  // TOTAL VES
  let ves = $("#ves").val();
  $("#totalVentaVES").val(sumaTotalPrecio * ves);
  $("#totalVES").val(sumaTotalPrecio * ves);
...
$(".administrarCotizaciones").on(
  ...
  function () {
    ...
    swal({
      ...
      if (result.value) {
        window.location =
          "index.php?ruta=administrarcotizaciones&idVenta=" + idVenta;
      }
    });
  }
);
...
$(".administrarCotizaciones").on(
  ...
  function () {
    ...
    window.open(
      "extensiones/tcpdf/pdf/cotizacion.php?codigo=" + codigoVenta,
      "_blank"
    );
  }
);
...
function tablaAdministrarVentas(
  ...
  $("#AdministrarVentas thead tr")
    .clone(true)
    .appendTo("#AdministrarVentas thead");
  ...
  $("#AdministrarVentas thead tr:eq(1) th").each(function (i) {
    ...
    if (
      ...
      $(this).html(
        '<input type="hidden" placeholder="Buscar por ' + title + '" />'
      );
    } else {
      $(this).html(
...
function tablaAdministrarCotizaciones(fechaInicial1, fechaFinal1) {
